...
**And what the review is actually for.** A program is not its text; it is the theory in the builders' heads, why the code has this shape, what was tried and abandoned, which parts are load-bearing (see the opening of [`running-a-review`](../../guides/running-a-review.md)). The primary product of a review is a second person who holds that theory, and defects are a valuable by-product. You are not that second person: the operator is, and so is whoever reads the thread next year. Two things follow for an agent specifically. **"I couldn't tell why this is shaped this way" is a first-class finding**, not a gap in your read, and it is often the most valuable thing you surface; raise it whenever you can say what a change does but not why it does it that way. And when the author answers a why, that answer belongs in the thread or in a comment in the code, not only in this session, which ends.
...
3. **Walk the four passes below** to gather candidate findings, then check each one in the code before it counts as a finding. If the answer turns on what a downstream function does with a value, open that function and read it; if it validates, drop the candidate silently rather than posting it hedged. A finding you haven't verified is a guess with a citation on it, and "I checked and I was wrong" costs nothing at this stage and costs the author a round trip after it.
...
5. **Work it finding by finding, with the human running the review.** Surface each finding as it clears: what it is, the evidence, whether it blocks. Size it and decide together against the Sizing a finding table in [`running-a-review`](../../guides/running-a-review.md): block, ask, nit or drop, with a nit still able to become a follow-up ticket or fold into what gets posted. **Reach for ask** rather than forcing a finding into block-or-nit: where the severity turns on something only the author or the product knows, how long a list gets or whether a path is user-facing, the finding is real and the missing fact is theirs. The agent proposes and the operator disposes; any back-and-forth with the author is the human's, downstream of this.
...
### 1. Correctness & tests
Does the change prove its own behaviour, the negative and gate branches included, against the real shape rather than a mock that only returns what you fed it? The test-craft to hold it to, from what to assert through to the one case where asserting a call is right, lives in [`test-writing`](../../guides/test-writing.md). Where a test looks like it wouldn't catch a regression, [`mutation-testing`](../../guides/mutation-testing.md) turns that from an opinion into a fact: break the line and name the test you expect to go red.
...
- **Brake:** `yagni` is the tag that most wants over-firing, and a reviewer who wields it that way becomes exhausting fast. Two call sites rarely tell you what varies, so the rule of three applies and duplication is cheaper than the wrong abstraction. A boundary you must genuinely swap and a published interface others depend on are constraints, not speculation. Raise it once, accept the answer, and never aim it at code the change merely touched.
...
- **Say what you didn't review.** Silence reads as coverage. Where a part of the change was beyond what you could follow, or needed domain knowledge you don't have, name that part and say a reader who holds it should look. A partial read stated honestly is a real review; a complete-looking read that quietly skipped the half you couldn't follow is worse than no review, because it spends the gate. Never soften a finding, or omit one, to make the read look finished.
